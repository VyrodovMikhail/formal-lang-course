...
from project.cfpq.tensor_algo import tensor_algo, start_tensor_algo


def test_tensor_algo():
    ...
    result = tensor_algo(grammar, graph)
    ...
    result_with_start_set = tensor_algo(grammar, graph, {0, 1})
    ...
    result_with_end_set = tensor_algo(grammar, graph, {0, 1}, {2})
    assert result_with_end_set == expected_result


def test_tensor_algo_from_files():
    var_S = Variable("S")
    var_S1 = Variable("S1")
    var_A = Variable("A")
    var_B = Variable("B")
    ...
    result = start_tensor_algo(path, "S", "tests/test_cfpq/graph.dot")
    assert result == expected_result
