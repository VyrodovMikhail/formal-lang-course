...
from project.cfpq.matrix_algo import matrix_algo, start_matrix_algo
...
def test_matrix_algo():
    ...
    result = matrix_algo(grammar, graph)
    ...
    result_with_start_set = matrix_algo(grammar, graph, {0, 1})
    ...
    result_with_end_set = matrix_algo(grammar, graph, {0, 1}, {2})
    assert result_with_end_set == expected_result


def test_matrix_algo_from_files():
    var_S = Variable("S")
    var_S1 = Variable("S1")
    var_A = Variable("A")
    var_B = Variable("B")
    ...
    result = start_matrix_algo(path, "S", "tests/test_cfpq/graph.dot")
    assert result == expected_result
